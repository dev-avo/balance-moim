...
- [ ] 7.5 사용자 간 상세 비교 페이지 구현
  - `/app/groups/[id]/compare/[userId]/page.tsx` 페이지 생성
  - 질문별로 나의 선택 vs 상대 선택 나란히 표시
  - 일치하는 질문은 초록색, 다른 질문은 빨간색
  - 전체 일치율 표시
  
- [ ] 7.6 사용자 간 상세 비교 API 구현
  - `GET /api/groups/:id/compare/:userId` 엔드포인트 생성
  - 두 사용자의 모든 공통 응답 조회
  - 질문별 선택 비교 데이터 반환
...
- [ ] 8.2 회원 탈퇴 확인 모달 구현
  - 생성한 모임 확인
  - "생성한 모임이 있습니다. 관리자 위임 또는 모임 삭제가 필요합니다" 경고
  - 최종 확인 체크박스
...
- [ ] 9.2 태그별 질문 조회 로직 추가
  - `GET /api/questions/random?tag=음식` 쿼리 파라미터 지원
  - `question_tag` 테이블 조인
  - 해당 태그를 가진 질문만 필터링
...
- [ ] 10.2 API 응답 캐싱 설정
  - 질문 통계는 5분간 캐싱 (`stale-while-revalidate`)
  - 태그 목록은 1시간 캐싱
  - Next.js `fetch` 옵션 활용
...
## 12. 반응형 디자인 완성
- [ ] 12.1 모바일 화면 최적화 (< 768px)
  - 세로 스크롤 레이아웃
  - 선택지 세로 배치
  - 햄버거 메뉴
  - 터치 영역 충분히 확보
  
- [ ] 12.2 태블릿 화면 최적화 (768px ~ 1024px)
  - 선택지 가로 배치
  - 사이드바 토글 기능
  
- [ ] 12.3 PC 화면 최적화 (≥ 1024px)
  - 고정 사이드바
  - 더 넓은 컨텐츠 영역
  - 호버 효과
